...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public R login(LoginDto loginDto) {
        ...
        if (validationResult.isHasError()) {
            loginRateLimitInterceptor.recordFailure(HttpContextUtils.getHttpServletRequest());
            return R.err(validationResult.getErrorMessage());
        }

        // 3. 生成令牌并返回用户信息
        User user = validationResult.getUser();
        loginRateLimitInterceptor.reset(HttpContextUtils.getHttpServletRequest());
    ...
    private LoginValidationResult validateUserCredentials(LoginDto loginDto) {
        ...
        String upgradedHash = PasswordUtil.upgradeHashIfLegacy(loginDto.getPassword(), user.getPwd());
