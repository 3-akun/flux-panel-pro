...
const (
	systemInfoInterval = 10 * time.Second
	systemInfoCacheTTL = 5 * time.Second
)
...
var (
	...
	cpuStatsOnce     sync.Once
)

func initCPUStats() {
	cpuStatsOnce.Do(func() {
...
func NewWebSocketReporter(serverURL string, secret string) *WebSocketReporter {
	...
	return &WebSocketReporter{
		...
		pingInterval:   systemInfoInterval,
...
// collectSystemInfo 收集系统信息（带短时缓存，避免频繁 syscall）
func (w *WebSocketReporter) collectSystemInfo() SystemInfo {
	cachedStatsMu.RLock()
	if time.Since(cachedStatsTime) < systemInfoCacheTTL {
		info := cachedSystemInfo
		cachedStatsMu.RUnlock()
		return info
	}
	cachedStatsMu.RUnlock()
...
// getCPUInfo 获取CPU信息（非阻塞，基于上次采样增量）
