...
CREATE TABLE `forward` (
  ...
  `auto_switch_enabled` int(10) NOT NULL DEFAULT '1',
  `auto_switch_fail_threshold` int(10) NOT NULL DEFAULT '2',
  `auto_switch_recover_threshold` int(10) NOT NULL DEFAULT '3',
  `health_check_interval_sec` int(10) NOT NULL DEFAULT '15',
  `health_check_timeout_ms` int(10) NOT NULL DEFAULT '3000',
  `preferred_target` varchar(255) DEFAULT NULL,
...
ALTER TABLE `forward`
  ...
  ADD INDEX `idx_tunnel_id` (`tunnel_id`);

ALTER TABLE `node`
  ADD INDEX `idx_secret` (`secret`);

ALTER TABLE `user`
  ADD UNIQUE INDEX `idx_user` (`user`);
