...
1. 配置不当或使用错误导致的服务异常或不可用；
2. 使用本项目引发的网络攻击、封禁、滥用等行为；
3. 服务器因使用本项目被入侵、渗透、滥用导致的数据泄露、资源消耗或损失；
4. 因违反当地法律法规所产生的任何法律责任。
...
本项目仅供个人学习与研究使用，仅限合法、合规用途。使用本项目产生的一切风险（含服务异常、封禁、数据损失、法律责任）均由使用者自行承担。

禁止将本项目用于任何违法或未经授权的行为，包括但不限于网络攻击、数据窃取、非法访问、公开代理售卖等。

作者不对因使用本项目导致的任何直接或间接损失承担责任，亦不提供任何形式的担保、承诺或技术支持。如不同意，请立即停止使用。

完整条款见上文 `免责声明与合规声明` 章节。
